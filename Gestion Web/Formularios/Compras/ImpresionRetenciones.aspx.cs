...
namespace Gestion_Web.Formularios.Compras
{
    public partial class ImpresionRetenciones : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    this.saldo = Request.QueryString["sdo"];
                    this.excel = Convert.ToInt32(Request.QueryString["xls"]); 
        ...
        private void generarReportes()
        {
            try
            {
                DataTable dtRetencion = this.contPagos.obtenerPagosRetencionesById(this.idRetencion);
                ...
                //Fecha Retencion                
                DateTime fechaRet = Convert.ToDateTime(dr["fecha"]);

                //Datos Cliente                
                string cliente = dr["Cliente"].ToString();
                DataTable dtCliente = this.contCliente.obtenerDireccionClienteNombreOrigen(cliente, 2);//2 = proveedor
                ...
                this.ReportViewer1.LocalReport.ReportPath = Server.MapPath("RetencionPagos.rdlc");
                ...
                ReportDataSource rds4 = new ReportDataSource("dsDocumentos", dtComprasCanceladas);
                ...
                ReportParameter param10 = new ReportParameter("ParamFecha", fechaRet.ToString("dd/MM/yyyy"));
        ...
        private void generarReporte2()
        {
            ...
            try
            {
                DataTable dtReten = this.controlador.obtenerCobrosRetenciones(Convert.ToDateTime(fechaD, new CultureInfo("es-AR")).ToString(), Convert.ToDateTime(fechaH, new CultureInfo("es-AR")).ToString(), this.suc, "P");

                this.ReportViewer1.ProcessingMode = ProcessingMode.Local;
                this.ReportViewer1.LocalReport.ReportPath = Server.MapPath("RetencionListado.rdlc");
                ReportDataSource rds = new ReportDataSource("dsRetenciones", dtReten);
                ...
                if (this.excel == 1)
                {
                    ...
                    String filename = string.Format("{0}.{1}", "DetalleComprasRetenciones", "xls");
